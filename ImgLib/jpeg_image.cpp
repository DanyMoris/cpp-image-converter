...
bool SaveJPEG(const Path& file, const Image& image) {
    ...
    /* Шаг 2: указываем приёмник данных */

    // Под Visual Studio путь может содержать символы, не представимые
    // в однобайтовой кодировке, поэтому открываем файл через _wfopen
#ifdef _MSC_VER
    ...
    // libjpeg ожидает строку из троек R, G, B, а Image хранит ещё и
    // прозрачность, поэтому каждую строку перекладываем во временный буфер
